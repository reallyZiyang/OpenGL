#version 330 core
#define MAX_TEXTURE_SIZE 4
struct Material
{
    vec3 baseColor;
    sampler2D diffuseTexture[MAX_TEXTURE_SIZE];
    sampler2D specularTexture[MAX_TEXTURE_SIZE];
    float shininess;
};
...
{
    vec3 texColor = vec3(texture(material.diffuseTexture[0], fragTexCoord));
    vec3 specularTexColor = vec3(texture(material.specularTexture[0], fragTexCoord));
...
vec3 calPointLight(PointLight light, vec2 fragTexCoord, vec3 fragPos, vec3 fragNormal)
{
    ...
    //点光源距离衰减公式
    float value = 1.0 / (1 + 0.14 * dis + 0.07 * dis * dis);
